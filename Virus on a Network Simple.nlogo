...
to setup-network
  ...
  if type-of-network = "random" [ create-RN num-links]
  if type-of-network = "lattice" [ create-lattice num-links]
  if type-of-network = "small_world" [
...
to layout
  ...
  ask turtles [face (patch 0 0) fd (distancexy 0 0) / 100 ]
end
...
to rewire-one
  ask one-of links [
    ...
    set node2 one-of turtles with [not link-neighbor? node1 and self != node1 and count link-neighbors < count turtles - 1]
